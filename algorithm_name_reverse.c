...
linked_list_t *sorting_function_name_rev(linked_list_t *first_node,
    linked_list_t *second_node)
{
    ...
    if (my_strcmp(first_node->name, second_node->name) >= 0){
        ...
        final_list->next =
            sorting_function_name_rev(first_node->next, second_node);
    } else {
        final_list = second_node;
        final_list->next =
            sorting_function_name_rev(first_node, second_node->next);
    }
    return (final_list);
}
...
int algorithm_tri_name_rev(linked_list_t **tete)
{
    ...
    algorithm_tri_name_rev(&left_node);
    algorithm_tri_name_rev(&right_node);
    *tete = sorting_function_name_rev(left_node, right_node);
}
